...
class Particle(Point):
    ...
    def reset_pos(self):
        self.set_pos(
            random.random() * WINDOWWIDTH,
            random.random() * WINDOWHEIGHT,
    ...
    def forward(self, d):
        ...
        x, y = self.position
        x = x + d * \
            math.cos(self.orientation) + gauss_noise(0, self.motion_noise)
        y = y + d * \
            math.sin(self.orientation) + gauss_noise(0, self.motion_noise)
    ...
    def turn_left(self, angle):
        self.orientation = (self.orientation + (angle + gauss_noise(0,
                                                                    self.turning_noise)) / 180. * math.pi) % (2 * math.pi)
    ...
    def dick(self):
        ...
        return [
            (x, y),
            (
                x + self.dick_length * math.cos(self.orientation),
                y + self.dick_length * math.sin(self.orientation)
            )
        ]
    ...
    def sense_direction(self, landmark):
        # TODO represent a landmark in NamedTuple
    ...
    def compute_jacobians(self, landmark):
        ...
        jacobian = np.array([[dx / d,   dy / d],
                             [-dy / d2, dx / d2]])
        adj_cov = jacobian.dot(landmark.sig).dot(
            np.transpose(jacobian)) + self.obs_noise
    ...
    def find_data_association(self, obs):
        ...
        for idx, landmark in enumerate(self.landmarks):
            ...
            p = multi_normal(np.transpose(
                np.array([obs])), predicted_obs, adj_cov)
            if p > prob:
                prob = p
