...
class Particle2(Particle):
    ...
    def __init__(self, x, y, orien, is_robot=False):
        ...
        self.control_noise = np.array(
            [[0.2, 0, 0], [0, 0.2, 0], [0, 0, (3.0 * math.pi / 180)**2]])
    ...
    def update(self, obs):
        ...
        for da in data_association:
            if da[1] > -1:
                ...
                predicted_obs, featurn_jacobian, pose_jacobian, adj_cov = self.compute_jacobians(
                    self.landmarks[da[1]])
                pose_cov = linalg.inv(np.transpose(pose_jacobian).dot(
                    linalg.inv(adj_cov).dot(pose_jacobian)) + linalg.inv(pose_cov))
                pose_mean = np.array([[self.x], [self.y], [self.orientation]]) + pose_cov.dot(np.transpose(
                    pose_jacobian)).dot(linalg.inv(adj_cov)).dot(np.transpose(np.array([da[0]])) - predicted_obs)
                new_pose = np.random.multivariate_normal(
                    pose_mean[:, 0], pose_cov)
        ...
        for da in data_association:
            if da[1] > -1:
                predicted_obs, featurn_jacobian, pose_jacobian, adj_cov = self.compute_jacobians(
                    self.landmarks[da[1]])
                self.weight *= multi_normal(np.transpose(
                    np.array([da[0]])), predicted_obs, adj_cov)
                self.update_landmark(np.transpose(np.array([da[0]])), da[
                                     1], predicted_obs, featurn_jacobian, adj_cov)
            else:
                self.weight *= da[2]
        prior = multi_normal(np.array([[self.x], [self.y], [
                             self.orientation]]), initial_pose, self.control_noise)
        prop = multi_normal(np.array([[self.x], [self.y], [
                            self.orientation]]), pose_mean, pose_cov)
    ...
    def compute_jacobians(self, landmark):
        ...
        pose_jacobian = np.array([[-dx / d, -dy / d, 0],
                                  [dy / d2, -dx / d2, -1]])
        adj_cov = feature_jacobian.dot(landmark.sig).dot(
            np.transpose(feature_jacobian)) + self.obs_noise
    ...
    def pre_compute_data_association(self, obs):
        ...
        for idx, landmark in enumerate(self.landmarks):
            ...
            predicted_obs, featurn_jacobian, pose_jacobian, adj_cov = self.compute_jacobians(
                landmark)
            pose_cov = linalg.inv(np.transpose(pose_jacobian).dot(linalg.inv(
                adj_cov).dot(pose_jacobian)) + linalg.inv(self.control_noise))
            x, y = self.position
            pose_mean = np.array([[x], [y], [self.orientation]]) + pose_cov.dot(np.transpose(
                pose_jacobian)).dot(linalg.inv(adj_cov)).dot(np.transpose(np.array([obs])) - predicted_obs)
            new_pose = np.random.multivariate_normal(pose_mean[:, 0], pose_cov)

            distance = euclidean_distance(
                landmark.position, (new_pose[0], new_pose[1]))
            direction = cal_direction(
                (new_pose[0], new_pose[1]), landmark.position)
            p = multi_normal(np.transpose(np.array([obs])), np.array(
                [[distance], [direction]]), adj_cov)
            if p > prob:
                prob = p
                landmark_idx = idx
